...
{
    //init
    int dk[6]={3,5,10,4,8,6};
    int ds[6]={0,0,0,0,0,0};
    int dp[6]={130,120,200,80,150,110};
    ...
    for(i=0;i<6;i++)
    {
        if(dk[i]!=0)printf("%d(o) %d\n",i+1,dp[i]);
        else if(dk[i]==0)printf("%d(o) %d\n",i+1,dp[i]);
    }
    //moneylost check
    lostflag=0;
    for(i=0;i<4;i++)
    ...
    else if(lostflag==0)
    {
        ...
        else if(dk[insert[5]]!=0)
        {
            if(dp[insert[5]-1]>moneyin)
            {
                printf("投入金額が足りません\n");
                printf("お金%d円が戻ります\n", moneyin);
            }
            else if(dp[insert[5]-1]<=moneyin)
            {
                printf("飲み物%dがでます\n",insert[5]);

                moneyback=moneyin-dp[insert[5]-1];
